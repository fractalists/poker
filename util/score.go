...
func hasThreeOfAKind(cards model.Cards) model.Cards {
	...
	for i := 14; i >= 2; i-- {
		if rankMemory[i] == 3 {
			...
			for _, card := range cards {
				if card.RankInt != i && needHighCardCount > 0 {
					result = append(result, card)
					needHighCardCount--
				}
			}
...
func hasTwoPair(cards model.Cards) model.Cards {
	...
	result := model.Cards{}
	for _, card := range cards {
		if card.RankInt == pairRanks[0] || card.RankInt == pairRanks[1] {
			result = append(result, card)
		}
	}
